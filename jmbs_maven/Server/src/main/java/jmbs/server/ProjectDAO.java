...
public class ProjectDAO extends DAO {
	...
	public boolean closeProject(int idproject) {
		...
		if (exists(idproject)) { // can be optimized
			...
			System.err
					.println("Project you are trying to close does not exists.");
			ret = false;
		}
		return ret;
	}
	...
	public Project createProject(String name, int iduser) {
		...
		if (!this.exists(name)) {
			...
			if (res) {// TODO: change creator when youyou is ready to implement
						// new interface
				try {
					ResultSet rs = getGeneratedKeys();
					ret = new Project(name, rs.getInt("idproject"),
							new UserDAO(con).getUser(iduser),
							Project.STATUS_OPENED, 0,
							Project.DEFAULT_EDIT_OPTION,
							Project.DEFAULT_SUPRESS_OPTION,
							Project.DEFAULT_ACCES_OPTION, currentTime);
